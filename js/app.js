(() => {
    ...
    class Popup {
        ...
        _openToHash() {
            ...
            let buttons = null;
            if (classInHash) buttons = document.querySelector(`[${this.options.attributeOpenButton} = "${classInHash}"]`) || document.querySelector(`[${this.options.attributeOpenButton} = "${classInHash.replace(".", "#")}"]`);
 ...
 */
    ...
    gsapWithCSS.registerPlugin(SplitText, ScrollTrigger_ScrollTrigger);
    const mm = gsapWithCSS.matchMedia();
    document.addEventListener("DOMContentLoaded", (() => {
        document.fonts.ready.then((() => {
            mm.add("(min-width: 1025px)", (() => {
                animateSplitTextOnScroll();
                animatePromoContent();
            }));
        }));
    }));
    let splitInstancesHome = [];
    let splitInstancesPromo = [];
    function animateSplitTextOnScroll() {
        splitInstancesHome.forEach((s => s.revert()));
        splitInstancesHome = [];
        const elements = document.querySelectorAll(".js-animated");
        const header = document.querySelector(".js-header");
        if (elements.length) elements.forEach((el => {
            const split = new SplitText(el, {
                type: "chars"
            });
            splitInstancesHome.push(split);
        ...
        if (header) gsapWithCSS.from(header, {
            autoAlpha: 0,
            duration: .8,
            ease: "power2.out"
        });
    ...
    function animatePromoContent() {
        splitInstancesPromo.forEach((s => s.revert()));
        splitInstancesPromo = [];
        const promo = document.querySelector(".js-animated-content");
        if (!promo) return;
        const topContent = promo.querySelector(".section-quest-main__top");
        const bottomContent = promo.querySelector(".section-quest-main__bot");
        if (topContent) {
            const splitTitle = new SplitText(topContent, {
                type: "chars"
            });
            splitInstancesPromo.push(splitTitle);
            gsapWithCSS.from(splitTitle.chars, {
                scrollTrigger: {
                    trigger: topContent,
                    start: "top 85%",
                    toggleActions: "play none none reverse"
        ...
        if (bottomContent) gsapWithCSS.from(bottomContent.children, {
            scrollTrigger: {
    ...
    document.addEventListener("DOMContentLoaded", (() => {
        const menuBnt = document.querySelector(".js-mobile-menu");
        const body = document.querySelector("body");
        if (menuBnt) menuBnt.addEventListener("click", (() => {
            body.classList.toggle("is-menu-active");
        }));
        const menuBntClose = document.querySelector(".js-mobile-menu-close");
        if (menuBntClose) menuBntClose.addEventListener("click", (() => {
            body.classList.remove("is-menu-active");
        }));
        const mobileMenu = document.querySelector(".mobile-menu");
        if (mobileMenu) mobileMenu.addEventListener("click", (e => {
            const target = e.target.closest("a");
            if (target) body.classList.remove("is-menu-active");
        }));
    }));
    window["FLS"] = true;
})();